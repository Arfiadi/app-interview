import { useRouter } from "next/router";
import { useState, useEffect } from "react";
import ProtectedRoute from "@/components/auth/ProtectedRoute";
import { useApi } from "@/hooks/useApi";
import Layout from "@/components/common/Layout";
import Button from "@/components/ui/Button";
import ScoreSummary from "@/components/results/ScoreSummary";
import ResultQuestionCard from "@/components/results/ResultQuestionCard";

interface ResultItem {
  question: string;
  similarity_score: number;
  user_answer?: string;
  keywords?: {
    user_keywords?: string[];
    missing_keywords?: string[];
  };
  feedback: string;
}

interface ResultData {
  overall_score: number;
  results: ResultItem[];
}

export default function ResultsPage() {
  const router = useRouter();
  const { saveHistory, getHistory } = useApi();
  const { session_id, result } = router.query;
  const [data, setData] = useState<ResultData | null>(null);
  const [saved, setSaved] = useState(false);

  useEffect(() => {
    if (result) {
      try {
        setData(JSON.parse(result as string));
      } catch (err) {
        console.error("Gagal parse hasil:", err);
      }
    } else if (session_id) {
      getHistory(session_id as string)
        .then((historyItem) => {
          if (historyItem && !historyItem.error) {
            setData(historyItem);
            setSaved(true);
          }
        })
        .catch((err) => {
          console.error("Gagal fetch history detail:", err);
        });
    }
  }, [result, session_id]);

  useEffect(() => {
    if (result && session_id && !saved) {
      saveHistory(session_id as string)
        .then(() => setSaved(true))
        .catch(() => console.warn("Gagal auto-save"));
    }
  }, [result, session_id, saved]);

  if (!data) {
    return (
      <ProtectedRoute>
        <Layout title="Menganalisis...">
          <div className="flex flex-col items-center justify-center min-h-[80vh] text-center px-4">
            <div className="w-12 h-12 border-[3px] border-gray-200 border-t-primary rounded-full animate-spin mb-6"></div>
            <h2 className="text-lg font-semibold text-primary">Sedang Menganalisis Jawaban...</h2>
          </div>
        </Layout>
      </ProtectedRoute>
    );
  }

  const { overall_score, results } = data;

  return (
    <ProtectedRoute>
      <Layout title="Laporan Hasil Interview">
        <div className="bg-background min-h-screen pb-20">
          
          <div className="bg-primary pb-32 pt-10 px-4 border-b border-white/10 no-print-bg">
              <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
                  <div className="text-white text-center md:text-left">
                      <h1 className="text-3xl font-bold mb-2">Laporan Evaluasi</h1>
                      <p className="text-blue-100 opacity-90">
                        Analisis performa sesi wawancara Anda.
                      </p>
                  </div>
                  
                  <div className="flex items-center gap-3">
                    <button 
                      onClick={() => window.print()}
                      className="no-print bg-white/20 hover:bg-white/30 text-white font-semibold px-5 py-2.5 rounded-full border border-white/20 flex items-center gap-2 text-sm transition-all shadow-md active:scale-95 cursor-pointer"
                    >
                      🖨️ Ekspor Laporan (PDF)
                    </button>
                    {saved && (
                      <div className="bg-white/10 backdrop-blur-md border border-white/20 px-4 py-2 rounded-full flex items-center gap-2 text-sm font-medium text-white shadow-lg">
                        <span>✅</span> Laporan Tersimpan
                      </div>
                    )}
                  </div>
              </div>
          </div>

          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-24">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  <div className="lg:col-span-4 lg:sticky lg:top-24 h-fit space-y-6 order-1 print-full-width">
                      <ScoreSummary score={overall_score} />
                      <div className="bg-white p-6 rounded-2xl shadow-soft border border-gray-100 hidden lg:block print:hidden">
                          <h3 className="font-bold text-primary mb-4">Langkah Selanjutnya</h3>
                          <div className="space-y-3">
                              <Button variant="primary" className="w-full justify-center" onClick={() => router.push('/pre-interview')}>Mulai Sesi Baru</Button>
                              <Button variant="outline" className="w-full justify-center" onClick={() => router.push('/history')}>Arsip Riwayat</Button>
                          </div>
                      </div>
                  </div>

                  <div className="lg:col-span-8 order-2 space-y-8 print-full-width">
                      <div className="flex items-center gap-3 mb-2 bg-white/50 backdrop-blur p-4 rounded-xl border border-gray-100 shadow-sm lg:bg-transparent lg:shadow-none lg:border-none lg:p-0 print:mt-6">
                          <div className="h-8 w-1 bg-secondary rounded-full"></div>
                          <div>
                              <h2 className="text-xl font-bold text-primary">Detail Umpan Balik</h2>
                              <p className="text-sm text-text-sub">Analisis per pertanyaan oleh AI Coach</p>
                          </div>
                      </div>
                      
                      {results.map((item, index) => (
                        <div key={index} className="print-avoid-break">
                          <ResultQuestionCard item={item} index={index} />
                        </div>
                      ))}

                      <div className="lg:hidden grid grid-cols-1 gap-3 pt-6 print:hidden">
                          <Button onClick={() => router.push('/pre-interview')}>Mulai Sesi Baru</Button>
                          <Button variant="outline" onClick={() => router.push('/history')}>Arsip Riwayat</Button>
                      </div>
                  </div>
              </div>
          </div>
        </div>
      </Layout>
    </ProtectedRoute>
  );
}